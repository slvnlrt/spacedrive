...
impl MacOSTestVolumeManager {
	...
	async fn create_ram_disk(&self, config: TestVolumeConfig) -> Result<TestVolume> {
		...
		let disk_path = String::from_utf8_lossy(&output.stdout).trim().to_string();
		...
		let output = tokio::process::Command::new("diskutil")
			.args(&["erasevolume", fs_type, &config.name, &disk_path])
			.output()
			.await
...
impl WindowsTestVolumeManager {
	...
	pub async fn check_privileges(&self) -> Result<()> {
		...
		if !output.status.success() {
			return Err(anyhow!(
				"Administrator privileges required for creating test volumes on Windows"
			));
		}
...
impl LinuxTestVolumeManager {
	...
	async fn create_loop_device(&self, config: TestVolumeConfig) -> Result<TestVolume> {
		...
		let output = tokio::process::Command::new("sudo")
			.args(&["mount", &loop_device, mount_point.to_str().unwrap()])
			.output()
			.await
	...
	pub async fn check_privileges(&self) -> Result<()> {
		...
		if !output.status.success() {
			return Err(anyhow!(
				"sudo privileges required for creating test volumes on Linux"
			));
		}
...
mod tests {
	...
	async fn test_volume_manager_creation() {
		...
		assert!(
			manager.check_privileges().await.is_ok() || manager.check_privileges().await.is_err()
		);
	}
