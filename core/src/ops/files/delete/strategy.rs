...
impl LocalDeleteStrategy {
	...
	pub async fn move_to_trash(&self, path: &Path) -> Result<(), std::io::Error> {
		// trash::delete is blocking, so wrap in tokio::task::spawn_blocking.
		// Native API handles cross-volume moves automatically.
		let path_buf = path.to_path_buf();
		tokio::task::spawn_blocking(move || {
			trash::delete(&path_buf).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))
		})
		.await
