...
impl FileTransferProtocolHandler {
	...
	fn get_all_allowed_paths(&self) -> Vec<PathBuf> {
		...
		if let Some(ctx) = &self.core_context {
			// Access library_manager field directly with blocking_read()
			// (ctx.libraries() is async, but we need sync access here)
			let library_manager_guard = ctx.library_manager.blocking_read();
			if let Some(library_manager) = library_manager_guard.as_ref() {
				// Get all active libraries using block_on for the async list() call
				...
				for library in library_list {
					...
					let location_manager = crate::location::LocationManager::new((*ctx.events).clone());
					if let Ok(locations) = futures::executor::block_on(
						location_manager.list_locations(&library)
					) {
	...
	/// SECURITY: Dynamically queries registered locations from all libraries.
	...
	async fn handle_incoming_transfer_request(
		...
		// SECURITY FIX (NET-01): Validate destination path is within allowed locations.
	...
	fn validate_path_access(&self, path: &std::path::Path, _requested_by: Uuid) -> bool {
		...
		// SECURITY FIX (NET-01): Validate path is within allowed locations.
...
mod tests {
	...
	fn test_is_path_allowed_rejects_paths_outside_allowed_locations() {
		...
		assert!(
			...
			"NET-01: Paths outside allowed locations MUST be rejected"
		);
		...
		#[cfg(windows)]
		{
			...
			assert!(
				...
				"NET-01: System paths MUST be rejected"
			);
		}
	...
	fn test_is_path_allowed_rejects_traversal_attempts() {
		...
		assert!(
			...
			"NET-01: Path traversal attempts MUST be rejected"
		);
	...
	fn test_is_path_allowed_denies_all_when_no_paths_configured() {
		...
		assert!(
			...
			"NET-01: When no allowed paths configured, ALL access should be denied (fail-safe)"
		);
	}
